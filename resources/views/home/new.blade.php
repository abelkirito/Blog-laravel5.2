...
@section('info')
    <title>{{\App\Http\Model\Config::all()->where('conf_title','网站标题')[0]['conf_content']}}-{{$field['art_title']}}</title>
...
@section('content')
            <article class="blogs">
                ...
                <div class="index_about">
                    ...
                    <div class="ad"> </div>
                    <div class="nextinfo">
                        @if($article['pre'])<p>上一篇：<a href="{{url('/a/'.$article['pre']['art_id'])}}">{{$article['pre']['art_title']}}</a></p>@else<p>上一篇：没有上一篇了</p> @endif
                        @if($article['next'])<p>下一篇：<a href="{{url('/a/'.$article['next']['art_id'])}}">{{$article['next']['art_title']}}</a></p>@else <p>下一篇：没有下一篇了</p>@endif
                    </div>
                    <div class="otherlink">
                        <h2>相关文章</h2>
                        <ul>
                            @foreach($data as $m)
                            <li><a href="{{url('./a/'.$m['art_id'])}}" title="{{$m['art_title']}}">{{$m['art_title']}}</a></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
